...
class Settings(BaseSettings):
    ...
    base_url: str = Field(default="https://registration.bogazici.edu.tr", validation_alias=AliasChoices("BOUN_BASE_URL", "BOUN_REGISTRATION_URL", "base_url"))
    quota_url: str = Field(default="https://registration.boun.edu.tr", validation_alias=AliasChoices("BOUN_QUOTA_URL", "BOUN_REGISTRATION_QUOTA_URL", "quota_url"))
    ...
    export_keep_last_n: int | None = Field(default=None, validation_alias=AliasChoices("BOUN_EXPORT_KEEP_LAST_N", "EXPORT_KEEP_LAST_N", "export_keep_last_n"))
    ...
    scraper_interval_seconds: int = Field(default=21600, validation_alias=AliasChoices("BOUN_SCRAPER_INTERVAL", "SCRAPER_INTERVAL", "scraper_interval_seconds"))
    ...
    def _resolve_secrets(self) -> "Settings":
        ...
        if "bountools.com" in self.base_url or "scraper." in self.base_url or self.base_url.strip() in ("", "/"):
            logger.warning(
                "base_url was set to self-referencing SPA domain (%s); resetting to upstream https://registration.bogazici.edu.tr",
                self.base_url,
            )
